...
<div id="app" :class="typeof weather.main != 'undefined' && weather.main.temp > 16 ? 'warm' : ''">
 <main>
  <div>
    <div class="h-4 w-9 text-black font-sans bg-white justify-items-center pt-6 ml-5">
      <input
        type="text" 
          class="search-bar" 
          placeholder="Search"
...
<img src="/map.jpg" alt=""/>
  ...
  <div class="bg-white pt-2">
      <h1 class="text-black text-xl text-bold pt-4 ml-5 font-ubuntu">{{ weather.name }}, {{ weather.sys.country }}</h1>
      <h2 class="text-black-500 text-xs ml-5 font-ubuntu">Weather forecast.</h2>
      <h3 class="text-black-500 text-sm ml-5 font-ubuntu">Today, {{ dateBuilder() }}</h3>
    </div>
      ...
      <div class="bg-white pt-4 rounded-md">
        ...
        <div class="pt-6 h-5 w-4 flex-auto flex space-x-3">
          <div class="flex-auto flex-items-center">
            <h2 class="text-blue-600 font-bold">{{Math.round(weather.main.temp_max)}}°c</h2>
            <p class="text-black font-normal">Maximum</p>
          </div>
          <div class="flex-auto flex-items-center">
            <h3 class="text-blue-600 font-bold">{{Math.round(weather.main.temp_min)}}°c</h3>
            <p class="text-black font-normal">Minimum</p>
          </div>
        </div>
      </div>
      <div class="font-medium ml-10 pt-4 rounded-md">
        <h1 class="text-black font-roboto align-center">Additional Parameters</h1> 
        <p class="text-blue font-normal">Wind {{Math.round(weather.main.wind_deg)}}</p>
        <p class="text-blue font-normal">pressure {{Math.round(weather.main.pressure)}}Pa</p>
        <p class="text-blue font-normal">Precipitation {{Math.round(weather.main.precipitation)}}</p>
        </div>
    </div>
